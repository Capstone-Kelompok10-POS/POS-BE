...
func MembershipCardRoutes(e *echo.Echo, db *gorm.DB, validate *validator.Validate) {
	membershipCardRepository := repository.NewMembershipCardRepository(db)
	membershipCardService := services.NewMembershipCardService(membershipCardRepository, validate)
	...
	membershipCardGroup.GET("/:id", MembershipCardHandler.PrintMembershipCard, middleware.AuthMiddleware("Cashier"))
}
